...
function calculateDeepScore(scores: DeepAuditResult["scores"]) {
...
function buildPatientOpportunities(scores: DeepAuditResult["scores"]) {
  const opportunities: {
    title: string;
...
export default function DeepReportPage() {
  const [deepAuditResult, setDeepAuditResult] =
    useState<DeepAuditResult | null>(null);

  const [generatedAt, setGeneratedAt] = useState("");

  useEffect(() => {
    const storedResult = sessionStorage.getItem("deepAuditResult");

    if (storedResult) {
      setDeepAuditResult(JSON.parse(storedResult));
    }
      ...
      <main className="flex min-h-screen items-center justify-center bg-slate-50 px-6">
        <div className="rounded-2xl bg-white p-8 text-center shadow-sm">
          ...
          <p className="mt-3 text-slate-600">
            Please run a deep scan from your quick audit report.
          </p>
        </div>
      </main>
    );
      ...
      <div className="mx-auto max-w-6xl px-6">
        <section className="rounded-3xl bg-white p-8 shadow-sm">
          ...
          <div className="mt-2 space-y-1 text-sm text-slate-500">
            <p>Website analyzed: {deepAuditResult.websiteUrl}</p>
